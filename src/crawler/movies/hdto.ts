...
import { MixDrop, VidCloud, MegaCloud } from '../../extractors';

class HDTO {
    readonly name = 'HDTO';
    protected baseUrl = 'https://1hd.to'; //https://1hd.sx/ -- maybe test url
    protected classPath = 'MOVIES.HDTO';
    ...
    private fetchSlider = ($: CheerioAPI, slider: ISlider[]) => {
        ...
        $('#slider > div.swiper-wrapper > div.swiper-slide').each((_, el) => {
            const image = $(el).find('.slide-cover > img').attr('src')!;
            //const match = image.match(sliderImageRegex)!;
            const title = $(el).find('.slide-element > .is-caption > .heading-xl').attr('title')!;
            const movieDetail = $(el).find('.slide-element > .is-caption > .description').text()!;
